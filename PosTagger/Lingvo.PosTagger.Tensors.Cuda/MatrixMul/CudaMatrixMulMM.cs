...
namespace Lingvo.PosTagger.Tensors.Cuda.MatrixMul
{
    /// <summary>
    /// 
    /// </summary>
    public static class CudaMatrixMulMM
    {
        ...
        {
            if ( a.Sizes[ 1 ] != c.Sizes[ 1 ] || b.Sizes[ 2 ] != c.Sizes[ 2 ] || a.Sizes[ 2 ] != b.Sizes[ 1 ] ) throw (new InvalidOperationException( "Size mismatch" ));
        ...
        private static void GemmOp( TSCudaContext context, BlasOp transA, BlasOp transB, float alpha, Tensor a, Tensor b, float beta, Tensor c )
        {
            ...
            using ( Util.PooledObject<CudaBlas> blas = context.BlasForTensor( c ) )
            {
                ...
                if ( c.ElementType == DType.Float32 )
                {
                    ...
                    CublasStatus statusF32 = CudaBlasNativeMethods.cublasSgemm_v2( blas.Value.CublasHandle, transa, transb, m, n, k, ref alpha, aPtrSingle, lda, bPtrSingle, ldb, ref beta, cPtrSingle, ldc );
                    if ( statusF32 != CublasStatus.Success )
                    {
                        throw (new CudaBlasException( statusF32 ));
                    }
                }
                else if ( c.ElementType == DType.Float64 )
                {
                    ...
                    CublasStatus statusF64 = CudaBlasNativeMethods.cublasDgemm_v2( blas.Value.CublasHandle, transa, transb, m, n, k, ref alphaDouble, aPtrDouble, lda, bPtrDouble, ldb, ref betaDouble, cPtrDouble, ldc );
                    if ( statusF64 != CublasStatus.Success )
                    {
                        throw (new CudaBlasException( statusF64 ));
                    }
                }
                else
                {
        ...
        private static void GemmOpBatch( TSCudaContext context, BlasOp transA, BlasOp transB, float alpha, Tensor a, Tensor b, float beta, Tensor c )
        {
            ...
            using ( var blas = context.BlasForTensor( c ) )
            {
                ...
                if ( c.ElementType == DType.Float32 )
                {
                    ...
                    CublasStatus statusF32 = CudaBlasNativeMethods.cublasSgemmStridedBatched( blas.Value.CublasHandle, transa, transb, m, n, k, ref alpha, aPtrSingle, lda, stra, bPtrSingle, ldb, strb, ref beta, cPtrSingle, ldc, strc, batchSize );
                    if ( statusF32 != CublasStatus.Success )
                    {
                        throw (new CudaBlasException( statusF32 ));
                    }
                ...
                else if ( c.ElementType == DType.Float64 )
                {
                    ...
                    double alphaDouble = alpha;
                    double betaDouble  = beta;
                    CublasStatus statusF64 = CudaBlasNativeMethods.cublasDgemmStridedBatched( blas.Value.CublasHandle, transa, transb, m, n, k, ref alphaDouble, aPtrDouble, lda, stra, bPtrDouble, ldb, strb, ref betaDouble, cPtrDouble, ldc, strc, batchSize );
                    if ( statusF64 != CublasStatus.Success )
                    {
                        throw (new CudaBlasException( statusF64 ));
                    }
