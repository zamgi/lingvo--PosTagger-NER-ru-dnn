...
namespace Lingvo.PosTagger.Tensors.Cuda.DeviceCode
{
    /// <summary>
    /// 
    /// </summary>
...
";
        ...
        public Tensor Scatter( Tensor result, Tensor src, int dim, Tensor indices )
        {
            try
            {
                ...
                if ( result.DimensionCount != src.DimensionCount ) throw (new InvalidOperationException( $"result and src must have same number of dimensions. result dim count = '{result.DimensionCount}', source dim count = '{src.DimensionCount}'" ));
