...
namespace Lingvo.PosTagger.Tensors
{
    ...
    {
        [M(O.AggressiveInlining)] public static long ElementCount( long[] sizes )
        {
            if ( sizes.Length == 0 )
            {
                return (0);
        ...
        [M(O.AggressiveInlining)] public static long GetStorageSize( long[] sizes, long[] strides )
        {
            long offset = 0;
            for ( int i = 0; i < sizes.Length; ++i )
            {
        ...
        [M(O.AggressiveInlining)] public static long[] GetContiguousStride( long[] dims )
        {
            long acc = 1;
