...
namespace System.Threading
{
    ...
    {
        public static Task ForEachAsync_< T >( this List< T > seq, int maxDegreeOfParallelism, CancellationToken ct, Func< T, CancellationToken, Task > seqItemFunc ) => ForEachAsync( (IReadOnlyCollection< T >) seq, maxDegreeOfParallelism, ct, seqItemFunc );
        public static async Task ForEachAsync< T >( this IReadOnlyCollection< T > seq, Func< T, int, CancellationToken, Task > seqItemFunc, int? maxDegreeOfParallelism = null, CancellationToken ct = default )
        {
            if ( (seq == null) || !seq.Any() )
            {
                return;
            }
            if ( maxDegreeOfParallelism.HasValue && maxDegreeOfParallelism.Value <= 0 ) throw (new ArgumentException( nameof(maxDegreeOfParallelism) ));
            if ( seqItemFunc == null ) throw (new ArgumentException( nameof(seqItemFunc) ));
            //-----------------------------------------------------------//

            var dop = maxDegreeOfParallelism.GetValueOrDefault( Environment.ProcessorCount );
            ...
            using ( var finitaEvent = new AsyncWaitEvent( false ) )
            {
                ...
                var firstErrors       = default(Exception);
...
#pragma warning disable CS4014
                    ...
                    Task.Run( async () =>
                    {
                        try
                        {
                            await seqItemFunc( local_t, local_i, ct ).ConfigureAwait( false );
                        }
                        catch ( Exception ex )
                        {
                            Debug.WriteLine( ex );

                            if ( Interlocked.CompareExchange( ref firstErrors, ex, null ) == null )
                            {
                                finitaEvent.Set();
                            }
                            #region comm.
                        ...
                        finally
                        {
                            ...
                            if ( (Interlocked.Increment( ref processedSeqCount ) == totalSeqCount) /*|| (firstErrors != null)*/ )
                            {
                                finitaEvent.Set();
                            }
                        }
...
#pragma warning restore CS4014
        ...
        public static async Task ForEachAsync< T >( this IReadOnlyCollection< T > seq, int maxDegreeOfParallelism, CancellationToken ct, Func< T, CancellationToken, Task > seqItemFunc ) 
        {
            if ( (seq == null) || !seq.Any() )
            {
                return;
...
#pragma warning restore CS4014
        ...
        public static async Task ForEachAsync< T >( this ICollection< T > seq, int maxDegreeOfParallelism, CancellationToken ct, Func< T, CancellationToken, Task > seqItemFunc ) 
        {
            if ( (seq == null) || !seq.Any() )
            {
                return;
...
#pragma warning restore CS4014
        ...
        public static async Task ForEachAsync< T >( this IEnumerable< T > seq, int maxDegreeOfParallelism, CancellationToken ct, Func< T, CancellationToken, Task > seqItemFunc )
        {
            if ( seq == null )
            {
                return;
...
#pragma warning restore CS4014
        ...
        public static async Task ForEachAsync< T >( this IEnumerable< T > seq, int maxDegreeOfParallelism, CancellationToken ct, Func< T, int, CancellationToken, Task > seqItemFunc, int startIndex = 0 )
        {
            if ( seq == null )
            {
                return;
